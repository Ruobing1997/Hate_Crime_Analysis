---
title: "Capstone With GGanimation"
author: "Ruobing Wang"
date: "2019/7/18"
...
Here is an interesting animation for the relationship between population of LGBT and Hate Crime. So we can see the trend of the hate crime changing over years.
...
region <- read_csv("/Users/wangruobing/Desktop/caps/us census bureau regions and divisions.csv")
lgbt <- read_csv("/Users/wangruobing/Desktop/caps/GallupAnalytics_Export_20190718_150757.csv", skip = 7)
lgbt <- lgbt[2:8]
table_all <- table_all %>% 
  left_join(region, by = c("NAME" = "State")) %>% 
  left_join(lgbt, by = c("NAME" = "Geography", "year" = "Time"))

table_all <- table_all %>% mutate(LGBT_POP = `Yes, do`*total)
...
g <- ggplot(table_all) +
  geom_point(
    aes(
      x = `Yes, do`, y = hate_rate,
      colour = Region, size = LGBT_POP),
    alpha = 0.7
  ) +
  scale_x_log10() +
  labs(title = 'Year:{frame_time}', x = 'The number of LGBT', y = 'Hate Crime Rate') +
  transition_time(year) +
  ease_aes('linear')

animate(g, nframes = 200, duration = 30, fps = 20, end_pause = 5)
